...
const issuer='https://approval-test.cloudflareaccess.com',aud='commerce-test';
let privateKey,jwk,token,erpMock;const originalFetch=globalThis.fetch;let erpCalls=[];
before(async()=>{const pair=await generateKeyPair('RS256');privateKey=pair.privateKey;jwk=await exportJWK(pair.publicKey);jwk.kid='test-key';jwk.alg='RS256';token=await sign('approver@example.invalid');globalThis.fetch=async (input,init)=>{const url=String(input instanceof Request?input.url:input);if(url===issuer+'/cdn-cgi/access/certs')return Response.json({keys:[jwk]});erpCalls.push(url);if(erpMock)return erpMock(url,init);throw Error('ERP must not be called in these standalone tests');};});
...
function fixture(stock=3){
 ...
 const env={DB:binding,ASSETS:{fetch:async()=>new Response('asset')},STORE_OWNER:'tenant-a',COMMERCE_APPROVERS:'approver@example.invalid',ACCESS_ISSUER:issuer,ACCESS_AUDIENCE:aud,PUBLIC_ORDERS_ENABLED:'1',ORDER_RATE_LIMITER:{limit:async()=>({success:true})}};
 ...
 const call=async(path,method='GET',body,auth=false,extra={})=>{const req=new Request('https://shop.example.invalid'+path,{method,headers:{origin:'https://shop.example.invalid','content-type':'application/json',...(auth?{'cf-access-jwt-assertion':typeof auth==='string'?auth:token}:{}),...extra},...(body?{body:JSON.stringify(body)}:{})});const r=await worker.fetch(req,env,{});return {status:r.status,body:await r.json()};};

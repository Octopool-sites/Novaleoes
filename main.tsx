...
function Management() {
  ...
  useEffect(()=>{let active=true;fetch("/api/session").then(async r=>{if(r.status===401)return;const data=await r.json() as {error?:string;name:string;environment?:"staging"|"production"};if(!r.ok)throw Error(data.error||"Não foi possível conferir o acesso.");if(active){setName(data.name);setEnvironment(data.environment === "production" ? "production" : "staging");}}).catch(e=>active&&setError(e.message)).finally(()=>active&&setChecking(false));return()=>{active=false;};},[]);
  ...
  return <><Suspense fallback={<p>Carregando gestão…</p>}><CommerceAdmin name={name} environment={environment}/></Suspense><button className="management-signout" onClick={signOut}>Sair da gestão</button>{error&&<p role="alert">{error}</p>}</>;
}

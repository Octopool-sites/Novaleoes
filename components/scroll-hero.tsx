...
export default function ScrollHero({ products, onProduct }: { products: Product[]; onProduct: (product: Product) => void }) {
  ...
  useEffect(() => {
    ...
    return () => motion.removeEventListener("change", preference);
  }, []);
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(entries => {
      ...
      started = true;
      void import("./car-scene").then(async ({ mountCarScene }) => {
        if (disposed) return;
        const scene = await mountCarScene(target, () => !disposed && setReady(true), () => !disposed && setUnavailable(true));
        if (disposed) scene.dispose();
        else { controller.current = scene; scene.update(progress.current); }
    ...
    <div className="nl-hero-sticky">
      ...
      <div className="nl-hero-layout">
        <div className="nl-hero-copy" key={chapter}>
          ...
          {chapter === 0 ? <h1>{current.title}</h1> : <h2>{current.title}</h2>}
        ...
        <div className="nl-car-area">
          <div className="nl-orbit" aria-hidden="true" />
          <div className="nl-car-canvas" ref={host} role="img" aria-label="Carro ilustrativo em 3D: a carroceria e as rodas se separam durante a rolagem" />
          ...
          <span className={`nl-part-tag nl-part-tag-${chapter}`} hidden={chapter === 0 || !ready}><span />{["", "Suspensão", "Filtro de ar", "Correia dentada"][chapter]}</span>
